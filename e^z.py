...
# Вычисляем значения функции e^z
W = np.exp(Z)

# ПЕРВЫЙ СЛАЙД: ДЕЙСТВИТЕЛЬНАЯ ЧАСТЬ
fig1 = plt.figure(figsize=(14, 6))
fig1.suptitle('Функция f(z) = e^z: Действительная часть',
...
ax1.set_title('3D поверхность: Re(e^z)', fontsize=16, pad=15)
...
fig1.colorbar(surf1, ax=ax1, shrink=0.7, pad=0.1, label='Re(e^z)')

# Срез вдоль действительной оси (Im(z) = 0)
ax2 = fig1.add_subplot(122)
x_real_line = np.linspace(-2, 2, 400)
z_real_line = x_real_line + 0j
w_real_line = np.exp(z_real_line)

ax2.plot(x_real_line, w_real_line.real, 'b-', linewidth=3, label='Re(e^z), Im(z)=0')
ax2.plot(x_real_line, w_real_line.imag, 'r--', linewidth=3, label='Im(e^z), Im(z)=0')

ax2.set_title('Срез вдоль действительной оси (Im(z) = 0) → e^x',
              fontsize=16, pad=15)
...
fig2.suptitle('Функция f(z) = e^z: Мнимая часть',
             fontsize=18, fontweight='bold', y=0.98)
...
ax1.set_title('3D поверхность: Im(e^z)', fontsize=16, pad=15)
ax1.set_xlabel('Re(z)', fontsize=12, labelpad=10)
ax1.set_ylabel('Im(z)', fontsize=12, labelpad=10)
ax1.set_zlabel('Im(f(z))', fontsize=12, labelpad=10)
ax1.view_init(elev=35, azim=45)
fig2.colorbar(surf1, ax=ax1, shrink=0.7, pad=0.1, label='Im(e^z)')

# Срез вдоль мнимой оси (Re(z) = 0)
ax2 = fig2.add_subplot(122)
y_imag_line = np.linspace(-2, 2, 400)
z_imag_line = 0 + 1j * y_imag_line
w_imag_line = np.exp(z_imag_line)

ax2.plot(y_imag_line, w_imag_line.real, 'b-', linewidth=3,
         label='Re(e^z), Re(z)=0 → cos(y)')
ax2.plot(y_imag_line, w_imag_line.imag, 'r-', linewidth=3,
         label='Im(e^z), Re(z)=0 → sin(y)')

ax2.set_title('Срез вдоль мнимой оси (Re(z) = 0) → cos(y) + i·sin(y)',
              fontsize=16, pad=15)
